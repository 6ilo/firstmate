...
| `fm-fleet-ledger.sh`     | Turn the opt-in fleet activity ledger on or off and append its records for outside tools ([contract](fleet-ledger.md)) |
| `fm-fleet-ledger-lib.sh` | Producer-side gate that makes the fleet ledger a single file test while it is off  |
