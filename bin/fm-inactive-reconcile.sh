...
# unpushed copy is not reported upstream as ready, and a no-mistakes
# pre-validation `done: {summary}` (the pipeline handoff) is never published.
# The cadence path uses fm-crew-state.sh, which applies the same gate: the
# handoff, and a CI-ready or direct-PR/local-only done whose head lives only in
# the disposable copy, read blocked and are not a terminal inactive outcome.
