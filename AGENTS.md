...
A clear or compaction re-emit is slim: it gives the context files only as paths and omits status tails and held or ready backlog rows, so read a named file when the turn needs it, or print the full re-emit with `bin/fm-session-start.sh --reemit --full`.
...
In no-mistakes mode the earlier `done [at=<epoch>]: {summary}` is the pipeline handoff, never a delivery: the same owner refuses it, so `bin/fm-crew-state.sh` reads it blocked until you start validation and a secondmate never publishes it upstream.
